...
- **Must work when opened directly from the filesystem, with no server.** Don't introduce a dependency (e.g. a `fetch()` of a local file) that only works over `http://` and breaks under `file://`. If a change needs to load something dynamically, make sure double-clicking `index.html` still works.
