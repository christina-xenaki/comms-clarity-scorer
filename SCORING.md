...
- **Clustering.** When enough matches exist (`clusteringMinMatches`, default 3) and one paragraph holds an outsized share of them (`clusteringShareThreshold`, default 40%), the buzzword card adds a line like *"6 of your 11 buzzwords are in paragraph 3."* Paragraphs are detected by blank-line breaks in the pasted text. **Known limitation:** this is currently wired up for the buzzword card only (no other check gets a clustering line), and the two thresholds are hardcoded in `js/render.js` rather than read from `config/config.json`'s `thresholds` object — they happen to match the config defaults above, but unlike every other threshold in this document, editing `clusteringMinMatches` or `clusteringShareThreshold` in the config file currently has no effect.
...
- Change a `penaltyPerPercent`/`penaltyPerGrade`/`penaltyPerWordOverAverage`/`penaltyEach`/`penaltyPerMismatch`/`penaltyPerIssueType`/`penaltyPerMatch`/`penaltyPerSharePoint` value to make the score fall faster or slower once past the target. (Exception: `clusteringMinMatches` and `clusteringShareThreshold` are currently hardcoded in `js/render.js`, not read from this file — see the "Clustering" known limitation in section A above.)
