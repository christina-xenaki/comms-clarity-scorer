...
- **`docs.html`** — A documentation viewer: renders README.md, SPEC.md, SCORING.md and COPY.md as styled HTML pages (pick one from its sidebar, or link directly with e.g. `docs.html?doc=scoring`) instead of sending readers to the raw markdown files, which GitHub Pages serves as plain text because of `.nojekyll` below. Its logic is `js/markdown.js` (a small dependency-free markdown-to-HTML converter) and `js/docs.js` (fetches the chosen file and renders it); its styling is `css/docs.css`. Internal links between the four docs (e.g. README's link to SPEC.md) are rewritten to stay inside the viewer.
